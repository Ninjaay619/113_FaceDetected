...
def draw_rect(img, pt1_idx, pt2_idx, color, label, raw_img=None):
    ...
    if not label or label.strip() == "":
        label = f"rect_{pt1_idx}_{pt2_idx}"
...
def draw_square_on_point(img, point_index, color, label, raw_img=None):
...
if img is None:
    ...
    for face_id, face in enumerate(faces):
        ...
        # 額頭以外其他臉部區塊
        draw_rect(img, 40, 4, (0, 0, 255), "right_cheek", raw_img=img_raw)      # 右臉頰
        draw_rect(img, 47, 12, (0, 0, 255), "left_cheek", raw_img=img_raw)      # 左臉頰
        draw_rect(img, 32, 52, (0, 255, 255), "philtrum", raw_img=img_raw)      # 人中
        draw_rect(img, 39, 32, (255, 0, 255), "right_nostril", raw_img=img_raw) # 右鼻翼
        draw_rect(img, 42, 34, (255, 0, 255), "left_nostril", raw_img=img_raw)  # 左鼻翼
        draw_square_on_point(img, 30, (0, 0, 0), "nose_tip", raw_img=img_raw)   # 鼻頭(脾)
        draw_square_on_point(img, 27, (0, 0, 0), "nose_root", raw_img=img_raw)  # 鼻根(肺)
